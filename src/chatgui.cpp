...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
    ...
    // GetChatLogicHandle() returns a reference to the ChatLogic instance, so we can call SendMessageToChatbot() directly.
     _panelDialog->GetChatLogicHandle().SendMessageToChatbot(std::string(userText.mb_str()));
}
    ...
    : wxScrolledWindow(parent, id)
{
    ...
    // By using std::make_unique we create an instance of ChatLogic and assign it to a unique_ptr.
    // The unique_prt, takes ownership of the object and it will manage its lifecycle.
...
ChatBotPanelDialog::~ChatBotPanelDialog()
{
    ...
    //// STUDENT CODE
    ////
    // Since _chatLogic is managed by a unique_ptr, we don't need to delete it manually
    // its lifecycle is managed by the unique_ptr.
    //delete _chatLogic;
