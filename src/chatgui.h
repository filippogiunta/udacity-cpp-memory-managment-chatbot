...
public:
    ...
    // NOTE: Instead of returning the raw object pointer, we return a reference to the object.
    // In this way we can make clear that the object is managed by a unique_ptr and also we avoid
    // the possibility to return a dangling pointer (if the object is deleted by the unique_ptr).
